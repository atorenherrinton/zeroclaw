...
original requirements, completion report and the latest `mcp-deadlines/` validation logs.
...
5. MCP parent deadlines, typed wrapper propagation and bounded detached recovery are now implemented. Preserve pre-write, post-write, stalled cleanup, failed handshake and direct-child reaping/no-replay tests. Finish spawned-descendant containment, reserve interactive capacity and add exact mutable-resource serialization.
