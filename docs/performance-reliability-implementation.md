...
working, tested subset is still `prerequisite-only`. Latest evidence is in the audit directory's `failure-evidence/` subdirectory;
`subprocess-lifecycle/`, `mcp-deadlines/`, `logging-bounds/`, and `policy-override/` preserve earlier continuations; `occurrence-read/`, `manual-claims/`, `orphan-recovery/`, `atomic/`, `finish/`, and
...
| 6. Typed connector uncertainty | prerequisite-only | A`delivery.rs`, A`deadline.rs`, R`agent/tool_execution.rs`, R`tools/delegate.rs`, R`cron/scheduler.rs` , T`mcp_client.rs`, T`mcp_tool.rs`, T`mcp_resources_tool.rs`, T`mcp_prompts_tool.rs`, R`agent/turn/mod.rs`, R`agent/turn/post_exec.rs`, R`agent/turn/events.rs`, R`agent/failure_evidence.rs` | `structured_delivery_and_deadline_errors_never_enter_string_recovery`; Telegram typed fault tests  MCP wrapper and direct/aggregate resource/prompt parent-deadline cases pass in `mcp-deadlines/`. Eight new failure-evidence regressions verify source text/data, failed artifact rejection, success HMAC retention, typed sibling precedence, tail stop, stalled auxiliary hooks and checkpoint failure. | Tool dispatch and agentic delegates preserve DeliveryFailure and DeadlineExceeded through anyhow instead of string retry recovery. MCP wrappers now preserve parent DeadlineExceeded and aggregate reads return errors on expiry. Generic ToolResult and scheduler execution remain string/bool based; unknown connector write errors still need typed receipts and reconciliation metadata throughout. No uncertain-write replay API exists. Returned batch results now survive later typed errors in current-turn history/SOP/HMAC views. Tool-returned cancellation remains typed. Whole-future cancellation before batch handoff, durable generic receipt ownership and external-platform confirmation remain open. |
...
| 8. Layered tool-output budgets | prerequisite-only | T`output_budget.rs`, R`agent/turn/mod.rs`, R`agent/tool_execution.rs` | `unicode_round_budget_does_not_persist_unretained_private_data`, `outcome_and_request_id_survive_large_json_result`, `manual_occurrence_receipt_survives_output_trimming`; passed in checkpoint tools suite (1,724 tests) | The repaired implementation explicitly disables resource persistence, contrary to early handoff report. Small top-level evidence now takes priority over excerpts: manual occurrence IDs and typed outcomes survive the 512-byte maximum-batch payload budget. Complete generic receipt/scope/error preservation, smaller configured budgets, and scoped overflow remain unfinished. Existing artifact writer needs scoped retention/access/reference ownership before reuse; no second store should be introduced. |
...
| 11. Bounded turns/deadlines | prerequisite-only | A`deadline.rs`, C`orchestrator/mod.rs`, R`agent/turn/mod.rs`, R`agent/tool_execution.rs`, R`tools/delegate.rs` , T`mcp_client.rs`, T`mcp_client/deadline_tests.rs`, `crates/zeroclaw-hardware/src/subprocess.rs`, `crates/zeroclaw-hardware/src/subprocess/lifecycle_tests.rs`, `crates/zeroclaw-hardware/src/loader.rs` | `nested_and_retry_work_cannot_extend_parent`, `phase_and_cancellation_survive_nested_deadlines`, typed dispatcher regression  Eleven added MCP regressions cover admission locks, resource/prompt wrappers, recovery reset/handshake/cleanup stalls, dropped recovery, and real synthetic stdio parent expiry with one effect and reaped direct children. Hardware subprocess admission, blocked-stdin cancellation, one-effect parent timeout, explicit cleanup, protocol byte limits, stderr backpressure and loaded-tool tests pass in `subprocess-lifecycle/`; real 10-second timeout also passes. | One monotonic parent deadline wraps turn/provider/tool/delegate/delivery phases; a real fake-provider delegate loop verifies child checkpoints cannot overwrite its parent channel journal while receipts remain shared; expired children are never polled. Timeout carries phase and started evidence. MCP connection setup, calls and capability operations now inherit the parent deadline, including lock and recovery waits. Typed expiry survives wrappers; initialized-notification failure prevents readiness. Existing detached recovery is bounded to 30 seconds plus 5 seconds cleanup and poisons on failure/abandonment without replay. Synthetic stdio tests prove direct-child reaping after initial-handshake and post-write expiry. Process-tree containment and remote-effect reversal remain unproven. Abort cleanup may continue up to 5 seconds for local durable persistence only. Larger background budgets/durable handoff remain incomplete. Hardware manifest subprocess calls now inherit the parent across all phases, bound stdin/stdout/exit exchange to 10 seconds with up to 5 seconds explicit cleanup, and retain sole direct-child ownership. No detached drain tasks; protocol lines cap at 1 MiB and stderr retains 512 bytes. Direct-child reaping is proven, descendant containment remains unfinished. |
...
| 13. Conflict-aware concurrency/backpressure | prerequisite-only | R`agent/tool_execution.rs`, C`orchestrator/mod.rs`, R`cron/scheduler.rs` | Parallel classification and reservation tests passed in checkpoint runtime/channels suites Actual parallel/sequential loop tests preserve completed sibling results and stop sequential tails on typed errors. | Known stateless read batches use buffered(4), max 128 calls. Unknown/shell/UI writes serialize within a batch. Cross-turn exact resource locks and global interactive worker reservation absent; separate pools are not proof against resource starvation. Terminal post-execution retention does not wait on hooks or draft progress; incomplete cards are best effort under backpressure. |
...
| 22. Replay/fault tests | prerequisite-only | C`telegram/delivery/tests.rs`, C`orchestrator/turn_journal.rs`, C`orchestrator/mod.rs`, I`tests/bounded_history_delivery.rs`, P`src/imessage_history/tests.rs` , T`mcp_client.rs`, T`mcp_client/deadline_tests.rs`, `crates/zeroclaw-hardware/src/subprocess/lifecycle_tests.rs`, R`agent/failure_evidence.rs`, R`agent/turn/events.rs` | 10 Telegram faults; 7 infra regressions; 11 synthetic Messages cases; actual dispatcher safe-queue recovery, rejected policy, duplicate ingress, cancellation after durable admission, hard abort and wake-up race tests  MCP synthetic subprocess deadline, re-handshake, direct-child reap and exactly-one-effect tests pass. Ten hardware regressions cover loader admission, blocked pipes, owned cancellation, cleanup failures and exact protocol byte budgets. Eight runtime evidence regressions use synthetic tools/providers/hooks/checkpoints; all pass in failure-evidence/. | Faults use local fake services and synthetic SQLite. Direct MCP child cancellation/deadline coverage now exists; whole-application termination across gateway ingress, delegates, scheduler and descendant processes still needs integration coverage. No real Messages history or outbound messages used. |
...
### Runtime failure evidence continuation

Failed ToolResult returns retain their source display text alongside the error
and their structured data in the existing outcome/SOP capture path. Source claims
such as delivered=true cannot create a delivered artifact on a failed result.
The added display envelope bounds accompanying source text at 32 KiB and its
error excerpt at 4 KiB before existing round/history budgets apply. Custom
structured data is not automatically copied into model text.

Batch dispatch now hands back each completed result or original typed error.
Sequential errors stop the undispatched tail; parallel reads retain completed
siblings in original call order. DeliveryFailure takes precedence over sibling
deadlines/cancellation without flattening its acknowledged-chunk evidence.
Tool-returned cancellation also remains typed. Completed results reach SOP
capture, ordered history and the existing HMAC collector before the terminal
error returns. No provider continuation or uncertain-write retry is introduced.

Terminal batches skip asynchronous post-tool hooks/draft progress, and incomplete
cards use best-effort nonblocking sends. Completed cards are not emitted twice.
Logs and SOP capture still retain completed results; post-execution log/progress
bodies now reuse the existing 4 KiB pre-redaction cap. Post-tool lifecycle
checkpointing happens after history retention, so checkpoint storage failure
cannot discard that completed in-memory round. Normal non-terminal batches keep
hooks/progress behavior.

This is current-turn evidence retention, not generic durable effect receipts.
HMACs attest runtime results, not external acknowledgement. Whole-future
cancellation before batch handoff can still lose in-memory results. Complete
encoded aggregate payload/metadata budgets, scoped overflow, durable generic
receipt ownership, and delegate/schedule/platform propagation remain unfinished.
No database, schema, dependency, policy fallback or retry path was added.

Final validation: **15,505 workspace tests passed, 0 failed,
24 ignored**, plus **55 personal-operations tests passed**. Eight new regressions
exercise the actual tool-loop/history/SOP/receipt boundary and failed artifact
projection. The production 10-second subprocess timeout also passes explicitly.
The workspace total excludes 5 embedded child-process summaries using
`count-tests.py`.

| Command | Result | Seconds |
| --- | --- | ---: |
| `cargo fmt --all -- --check` | pass | 4.04 |
| `cargo check --locked --workspace --all-targets` | pass | 0.67 |
| `cargo clippy --locked --workspace --all-targets -- -D warnings` | pass | 0.61 |
| `cargo test --locked --workspace --no-fail-fast -- --test-threads=4` | pass | 189.89 |
| `cargo clippy --locked -p zeroclaw-api -p zeroclaw-infra -p zeroclaw-runtime -p zeroclaw-channels -p zeroclaw-providers -p zeroclaw-log -p zeroclaw-config -p zeroclaw-tools -p zeroclaw-hardware -p zeroclaw-plugins --all-targets --all-features -- -D warnings` | pass | 87.31 |
| `cargo build --locked --release --workspace` | pass | 1.07 |
| `cargo test --locked -p zeroclaw-hardware --lib subprocess::tests::execute_timeout_kills_process_and_returns_error -- --exact --ignored` | pass | 10.34 |
| `cargo fmt --manifest-path tools/zeroclaw-personal-ops/Cargo.toml -- --check` | pass | 0.1 |
| `cargo test --locked --manifest-path tools/zeroclaw-personal-ops/Cargo.toml` | pass | 0.63 |
...
| `cargo clippy --locked --workspace --all-targets --all-features --keep-going -- -D warnings` | blocked: actual web assets missing | 2.31 |
| `cargo clippy --locked -p zeroclaw-gateway --all-targets --features a2a,channel-acp-server,channel-email,channel-linq,channel-nextcloud,channel-nostr,channel-wechat,channel-whatsapp-cloud,gateway-voice-duplex,observability-prometheus,plugins-wasm,schema-export,webauthn,whatsapp-web -- -D warnings` | pass | 0.37 |
| `bash scripts/ci/parallel_runtime_test_gate.sh` | pass | 251.2 |

Commands use the shared target, `CARGO_INCREMENTAL=0`,
`CARGO_PROFILE_DEV_DEBUG=0`, and `CARGO_PROFILE_TEST_DEBUG=0`.
Three native parallel repetitions each pass 4,230 runtime tests (3 ignored) and
1,571 channel tests (2 ignored), with 16 harness threads and no stack override.
Default strict workspace Clippy and ten affected all-features crates pass.
Whole-workspace all-features Clippy remains blocked by missing actual
`web/dist/index.html`; gateway Clippy passes with all features except embedded-web.
Three WASM channel fixture sender factories were migrated to the existing
API ingress test queue after all-features compilation exposed stale Tokio
senders. All-features validation now uses --keep-going and rejects any compiler
error beyond the missing-web-assets build failure. A validator path comparison
was normalized after it falsely rejected Cargo’s normalized diagnostic path;
this changed only the audit driver, and the extra validation was rerun. The earlier gate is preserved
in `before-wasm-fixture-sender-fix/`. The WASM runtime fixture was not executed:
its wasm32-wasip2 standard library is not installed; host fixture compilation is
covered by strict all-features Clippy. No target, placeholder asset or dependency
was installed. Both release builds are
build artifacts only.

Initial focused compilation caught a borrowed sender in the synthetic fixture.
The first behavioral run caught invalid multiline Fluent syntax; after repair,
the broader agent suite passed 972 tests (2 ignored). Later review found that a
sibling cancellation assertion did not detect an ordinary-error conversion.
Cancellation propagation and the assertions were repaired; a separate sequential
case was added. The first full gate was intentionally interrupted during test
compilation after format/check/default strict Clippy passed, and retained in
`before-cancellation-propagation-fix/`. The next full workspace run found one old cancellation-marker assertion; its
exact-one-terminal checks passed, and only the expected neutral message key was
updated. That failure is retained in `before-cancellation-marker-test-fix/`.
All final commands were rerun on frozen corrected source. The eight new
regressions and the original cancellation/card test pass in the full gate. Full-file
docs lint also passes. Exact commands, timings, initial failures, source hashes,
release hashes and preservation evidence are in `failure-evidence/`.


### Hardware subprocess lifecycle continuation

The existing manifest-loaded SubprocessTool now owns its direct child across
stdin, stdout, natural exit and error cleanup with kill-on-drop. The canonical
parent deadline wraps admission and all I/O; expired parents never spawn. A
10-second exchange ceiling includes stdin and natural exit, with at most 5
additional seconds for explicit kill/reap. Stderr drains alongside the exchange
without a spawned task, retaining 512 bytes; complete request/response protocol
lines cap at 1 MiB. Requests are bounded before spawn and responses before parse.

A parsed result requires successful child exit. Later exit failure or local
timeout returns failure while preserving that result's text/data at the runner
boundary. Generic runtime failure projection still discards these outputs; this
is not completion of generic receipt propagation. Parent cancellation remains a
typed deadline and never proves non-delivery. No retry, new process registry,
dependency, schema, config, installation or descendant-containment claim.

Final validation: **15,497 workspace tests passed, 0 failed,
24 ignored**, plus **55 personal-operations tests passed**. Hardware unit
coverage is 103 passed and 1 ignored; ten regressions were added. The normally
ignored production 10-second timeout test was also run explicitly and passed.
The workspace total excludes 5 embedded child-process summaries using
`count-tests.py`.

| Command | Result | Seconds |
| --- | --- | ---: |
| `cargo fmt --all -- --check` | pass | 3.94 |
| `cargo check --locked --workspace --all-targets` | pass | 1.59 |
| `cargo clippy --locked --workspace --all-targets -- -D warnings` | pass | 1.22 |
| `cargo test --locked --workspace --no-fail-fast -- --test-threads=4` | pass | 190.89 |
| `cargo clippy --locked -p zeroclaw-api -p zeroclaw-infra -p zeroclaw-runtime -p zeroclaw-channels -p zeroclaw-providers -p zeroclaw-log -p zeroclaw-config -p zeroclaw-tools -p zeroclaw-hardware --all-targets --all-features -- -D warnings` | pass | 117.05 |
| `cargo build --locked --release --workspace` | pass | 262.97 |
| `cargo test --locked -p zeroclaw-hardware --lib subprocess::tests::execute_timeout_kills_process_and_returns_error -- --exact --ignored` | pass | 10.63 |
| `cargo fmt --manifest-path tools/zeroclaw-personal-ops/Cargo.toml -- --check` | pass | 0.11 |
| `cargo test --locked --manifest-path tools/zeroclaw-personal-ops/Cargo.toml` | pass | 0.79 |
| `cargo clippy --locked --manifest-path tools/zeroclaw-personal-ops/Cargo.toml --all-targets -- -D warnings` | pass | 0.16 |
| `cargo build --locked --release --manifest-path tools/zeroclaw-personal-ops/Cargo.toml` | pass | 0.16 |
| `cargo clippy --locked --workspace --all-targets --all-features -- -D warnings` | blocked: actual web assets missing | 22.87 |
| `cargo clippy --locked -p zeroclaw-gateway --all-targets --features a2a,channel-acp-server,channel-email,channel-linq,channel-nextcloud,channel-nostr,channel-wechat,channel-whatsapp-cloud,gateway-voice-duplex,observability-prometheus,plugins-wasm,schema-export,webauthn,whatsapp-web -- -D warnings` | pass | 7.41 |
| `bash scripts/ci/parallel_runtime_test_gate.sh` | pass | 178.44 |

Commands use the shared target, `CARGO_INCREMENTAL=0`,
`CARGO_PROFILE_DEV_DEBUG=0`, and `CARGO_PROFILE_TEST_DEBUG=0`.
Three native parallel repetitions each pass 4,222 runtime tests (3 ignored) and
1,571 channel tests (2 ignored), with 16 harness threads and no stack override.
Strict affected all-features Clippy now includes hardware alongside the previous
eight crates. Whole-workspace all-features Clippy remains blocked by missing
actual `web/dist/index.html`; the gateway feature subset excludes embedded-web.
No placeholder assets or dependencies were installed.

An initial compile ran before the new test module file existed. The next fixture
compile found an ambiguous `as_ref()` assertion; it was replaced with the existing
`as_str()` API, and the structured fixture was aligned with ToolOutput's canonical
`data` wire key. The first full gate then caught a fixture-startup race: a 500 ms local timeout
could kill the child before its script wrote a PID marker. The test now observes
startup while execution is active and uses a 3-second fixture budget, retaining
reaping and timeout assertions. Production budgets are unchanged. The entire
gate was rerun; initial evidence is in `before-fixture-startup-fix/`. Full-file docs lint passes. All executable source hashes match
the frozen gate input; only this matrix's evidence is updated afterward. Exact
commands, timings, binary hashes and preservation evidence are in
`subprocess-lifecycle/`.

### MCP parent deadlines and bounded recovery continuation

MCP connection setup, calls, resource and prompt operations now inherit the
canonical parent deadline across config/capability locks, queued admission and
recovery waits. Expired admission prevents polling new work or spawning a server.
The existing tool wrappers retain `DeadlineExceeded` for the runtime. Aggregate
catalogs return `Result` and cannot convert parent expiry into an empty success.
Connection admission futures are boxed at the MCP boundary so the deadline
wrappers do not amplify native stack usage in nested agent/delegate assembly.

The existing request lifecycle and recovery barrier still own write uncertainty.
Detached connection repair has a 30-second budget and at most 5 additional
seconds of cleanup. Failure or abandonment poisons admission. Initialized
notification writes are timed and must succeed. Synthetic stdio tests cover
initial-handshake cancellation, direct-child reaping, re-handshake and exactly
one side effect after a post-write deadline. No request is replayed after an
uncertain write. The existing explicit-abort fixture remains covered separately.

This does not add a ledger or claim generic external-effect receipts, remote
rollback, process-tree containment, interactive capacity, mutable-resource locks,
or coordinated process drain/readiness. Those backlog boundaries remain open.
Final validation: **15,487 workspace tests passed, 0 failed,
24 ignored**, plus **55 personal-operations tests passed**. Eleven MCP
regressions were added; the focused MCP suite passes 226 tests. The workspace
total excludes 5 embedded child-process summaries using `count-tests.py`.

| Command | Result | Seconds |
| --- | --- | ---: |
| `cargo fmt --all -- --check` | pass | 3.86 |
| `cargo check --locked --workspace --all-targets` | pass | 36.47 |
| `cargo clippy --locked --workspace --all-targets -- -D warnings` | pass | 44.16 |
| `cargo test --locked --workspace --no-fail-fast -- --test-threads=4` | pass | 290.9 |
| `cargo clippy --locked -p zeroclaw-api -p zeroclaw-infra -p zeroclaw-runtime -p zeroclaw-channels -p zeroclaw-providers -p zeroclaw-log -p zeroclaw-config -p zeroclaw-tools --all-targets --all-features -- -D warnings` | pass | 88.27 |
| `cargo build --locked --release --workspace` | pass | 299.02 |
| `cargo fmt --manifest-path tools/zeroclaw-personal-ops/Cargo.toml -- --check` | pass | 0.11 |
| `cargo test --locked --manifest-path tools/zeroclaw-personal-ops/Cargo.toml` | pass | 0.9 |
| `cargo clippy --locked --manifest-path tools/zeroclaw-personal-ops/Cargo.toml --all-targets -- -D warnings` | pass | 0.16 |
| `cargo build --locked --release --manifest-path tools/zeroclaw-personal-ops/Cargo.toml` | pass | 0.17 |
| `cargo clippy --locked --workspace --all-targets --all-features -- -D warnings` | blocked: actual web assets missing | 2.71 |
| `cargo clippy --locked -p zeroclaw-gateway --all-targets --features a2a,channel-acp-server,channel-email,channel-linq,channel-nextcloud,channel-nostr,channel-wechat,channel-whatsapp-cloud,gateway-voice-duplex,observability-prometheus,plugins-wasm,schema-export,webauthn,whatsapp-web -- -D warnings` | pass | 38.91 |
| `bash scripts/ci/parallel_runtime_test_gate.sh` | pass | 227.12 |
...
1,571 channel tests (2 ignored), with 16 harness threads and no stack override.
Whole-workspace all-features Clippy remains blocked by missing actual
`web/dist/index.html`; the gateway feature subset excludes only embedded-web.
No placeholder assets or dependencies were installed.

Initial compile attempts caught an error-preservation match arm inserted into
the embedded-resource error branch; it was corrected before focused and final
validation. Docs lint caught one duplicate blank line, which was removed without
changing executable source; its before/after hashes are recorded separately.
The first workspace gate reproduced a native stack overflow in the existing
routed-agentic-delegate test. Nested MCP admission deadline futures had increased
the delegate assembly stack. Boxing the two MCP admission futures reduced their
measured sizes from 3,016/6,624 bytes to 440/176 bytes on this host;
a regression enforces bounded public future sizes. The original delegate test
passes without changing it or increasing the native stack. The complete gate
was rerun after this fix; initial gate evidence is in `before-stack-fix/`.
Full-file docs lint passes. Final executable source hashes match the frozen gate
input; only this matrix's evidence is updated after the Rust gate. Logs, exact
commands, timings, binary hashes and preservation evidence are in `mcp-deadlines/`.
...
The isolated policy milestone adds optional `catch_up_once`, `skip`, and
`reconcile` overrides to the existing imperative cron row. CLI `cron update`,
`cron_update`, HTTP PATCH and RPC `cron/patch` preserve omission and support
explicit inheritance reset. Declarative jobs still resolve current config;
database patches to their policy are refused. Job updates read, mutate and
return their result within one FULL-synchronous writer transaction. Quarantine
remains disabled even when a patch requests enabled. Startup skip/reconcile
refuses stale policy or ownership, including legacy null-source rows.

Migration, corrupt policy, config shadow ownership, atomic rollback, owner
rejection, claim/receipt preservation, restart disposition, and actual
CLI/tool/authenticated HTTP/RPC boundaries are tested. The final source passes
15,465 workspace tests, zero failures, 24 ignored, plus 55
standalone personal-operations tests. The first full run also passed; final
validation was repeated after the legacy-source and transactional-return review
fixes. Initial compiler wiring errors were corrected and retained in the audit
logs. The count excludes five embedded child-process summaries, using the
same per-target method as the preceding milestone. No runtime test failure remained.

| Command | Result | Seconds |
| --- | --- | ---: |
| `cargo fmt --all -- --check` | pass | 3.83 |
| `cargo check --locked --workspace --all-targets` | pass | 29.44 |
| `cargo clippy --locked --workspace --all-targets -- -D warnings` | pass | 34.62 |
| `cargo test --locked --workspace --no-fail-fast -- --test-threads=4` | pass | 270.29 |
| `cargo clippy --locked -p zeroclaw-api -p zeroclaw-infra -p zeroclaw-runtime -p zeroclaw-channels -p zeroclaw-providers -p zeroclaw-log -p zeroclaw-config -p zeroclaw-tools --all-targets --all-features -- -D warnings` | pass | 74.72 |
| `cargo build --locked --release --workspace` | pass | 266.14 |
| `cargo fmt --manifest-path tools/zeroclaw-personal-ops/Cargo.toml -- --check` | pass | 0.15 |
| `cargo test --locked --manifest-path tools/zeroclaw-personal-ops/Cargo.toml` | pass | 0.87 |
| `cargo clippy --locked --manifest-path tools/zeroclaw-personal-ops/Cargo.toml --all-targets -- -D warnings` | pass | 0.15 |
| `cargo build --locked --release --manifest-path tools/zeroclaw-personal-ops/Cargo.toml` | pass | 0.15 |
| `cargo clippy --locked --workspace --all-targets --all-features -- -D warnings` | blocked: missing actual embedded-web assets | 14.27 |
| `cargo clippy --locked -p zeroclaw-gateway --all-targets --features a2a,channel-acp-server,channel-email,channel-linq,channel-nextcloud,channel-nostr,channel-wechat,channel-whatsapp-cloud,gateway-voice-duplex,observability-prometheus,plugins-wasm,schema-export,webauthn,whatsapp-web -- -D warnings` | pass | 30.58 |
| `bash scripts/ci/parallel_runtime_test_gate.sh` | pass | 251.78 |

Builds use the shared target with `CARGO_INCREMENTAL=0`,
`CARGO_PROFILE_DEV_DEBUG=0`, and `CARGO_PROFILE_TEST_DEBUG=0`. The native parallel
gate repeats the full runtime/channel suites three times at 16 harness threads.
Whole-workspace all-features Clippy still requires the missing actual web assets;
no placeholders or dependency installation were used to bypass that requirement.
The full reliability backlog remains incomplete. Nothing was installed,
restarted, integrated into master, or pushed. Evidence is in `policy-override/`.
...
cases. `manual-claims/checkpoint-workspace-tests.counts.json` excludes five
embedded child-process summaries; `manual-claims/focused-evidence.json` names
the HTTP, RPC, tool, cancellation, ownership and malformed-receipt boundaries.

Manual invocations claim durable ownership before execution, checkpoint before
notification, and atomically complete receipt/history/lock state. Keyed replay
returns original evidence, including uncertainty, without execution or notification.
Missing or malformed receipt storage never becomes proof of non-delivery.
Top-level manual IDs and typed outcomes take priority over excerpts, including
512-byte maximum-batch budgets and exact-fit evidence. This repairs existing
cron ownership in the transitional runtime crate; crate decomposition remains
separate work. The full 1–23 backlog is still incomplete.

Validation commands and final release evidence are recorded in
`manual-claims/checkpoint-validation.jsonl`, `final-validation.jsonl` and
`validation.jsonl`. Earlier runs passed 15,442 and 15,443 tests before the final
receipt changes. The initial default Clippy failed on a generic error macro and
a direct task spawn; both now use the required project APIs. Earlier focused
failures exposed a masked SQLite decoding error and a fixture that attempted a
new invocation before resolving its prior quarantine. Both were repaired while
preserving the no-replay assertions; all original logs remain available.

| Exact command | Result | Seconds |
| --- | --- | ---: |
| `cargo fmt --all -- --check` | pass | 3.78 |
| `cargo check --locked --workspace --all-targets` | pass | 35.33 |
| `cargo test --locked --workspace --no-fail-fast -- --test-threads=4` | 15,444 passed; 0 failed; 24 ignored | 285.62 |
| `cargo test --locked -p zeroclaw-runtime --lib cron:: -- --test-threads=4` | 210 passed; 0 failed | 45.3 |
| `ZEROCLAW_PARALLEL_TEST_RUNS=1 ZEROCLAW_PARALLEL_TEST_THREADS=16 ZEROCLAW_PARALLEL_TEST_SCOPE=all bash scripts/ci/parallel_runtime_test_gate.sh` | runtime 4,205 passed / 3 ignored; channels 1,571 passed / 2 ignored | 59.43 |
| `cargo clippy --locked --workspace --all-targets -- -D warnings` | pass | 42.6 |
| `cargo clippy --locked --workspace --all-targets --all-features -- -D warnings` | blocked: missing actual `web/dist/index.html` | 22.82 |
| `cargo clippy --locked -p zeroclaw-api -p zeroclaw-infra -p zeroclaw-runtime -p zeroclaw-channels -p zeroclaw-providers -p zeroclaw-log -p zeroclaw-config -p zeroclaw-tools --all-targets --all-features -- -D warnings` | pass | 82.13 |
| `cargo build --locked --release --workspace` | pass | 270.41 |
| `cargo fmt --manifest-path tools/zeroclaw-personal-ops/Cargo.toml -- --check` | pass | 0.1 |
| `cargo test --locked --manifest-path tools/zeroclaw-personal-ops/Cargo.toml` | 55 passed; 0 failed; synthetic Messages databases only | 0.89 |
| `cargo clippy --locked --manifest-path tools/zeroclaw-personal-ops/Cargo.toml --all-targets -- -D warnings` | pass | 0.15 |
| `cargo build --locked --release --manifest-path tools/zeroclaw-personal-ops/Cargo.toml` | pass | 0.16 |

The final release build includes every code change above. Code hashes in
`manual-claims/checkpoint-source-hashes.json` and `clippy-source-hashes.json`
were checked against the committed source. The earlier three-repeat parallel
gate passed in 323.39 seconds; its first runtime repetition preceded the final
receipt changes. The additional one-repeat gate above runs the final source.
`parallel-evidence.json` records each repetition separately.

All commands use the canonical `target/` directory, `CARGO_INCREMENTAL=0`,
`CARGO_PROFILE_DEV_DEBUG=0` and `CARGO_PROFILE_TEST_DEBUG=0`; release retains its
normal profile. Whole-workspace all-features Clippy still requires actual
embedded-web assets. Eight affected Rust crates pass all-features Clippy, and
the default full workspace passes strict Clippy. No frontend dependencies or
placeholder assets were installed. The documentation quality and local-link
checks passed; exact output is in `docs-quality-final.log` and `docs-links.log`.

### Previous scheduler recovery and missed-run continuation (September 7, `cd4caa8a0`)

That checkpoint workspace suite passed **15,431 tests, 0 failed, 24 ignored** in
...
The cron lock-owner column is additive, but older binaries do not enforce manual
invocation ownership or keyed replay. Do not run an older scheduler against active
or uncertain manual occurrences; retaining the column alone is not a safe rollback.
...
original requirements, completion report and the latest `failure-evidence/` validation logs.
...
2. Finish durable typed external-effect receipts through generic tools, delegates
   and schedules. Current-turn batch retention now preserves completed outcomes,
   source failure text/data and existing HMACs when a later typed error returns;
   preserve the failure-evidence tests. This does not persist evidence when the
   entire turn future is cancelled before batch handoff, or establish platform
   confirmation. Keep typed delivery/deadline/cancellation errors outside retry
   recovery and do not promote failed source claims into delivered artifacts.
...
5. MCP parent deadlines, typed wrapper propagation and bounded detached recovery are implemented. Hardware manifest subprocess tools now inherit the parent deadline, own/reap their direct child, drain stderr concurrently and bound protocol lines. Preserve their admission, cancellation, backpressure, byte-boundary, no-replay and cleanup tests. Finish spawned-descendant containment, reserve interactive capacity and add exact mutable-resource serialization. Generic runtime failure projection now retains plugin output reported before a later exit failure; the durable generic effect-receipt boundary remains unfinished.
6. Extend the now-separate typed notification state with platform receipt IDs and
   evidence-driven reconciliation. Imperative per-job missed-run policy overrides are now implemented; preserve their inheritance, source ownership, transaction, stale-policy and no-replay tests. Read-only operator occurrence queries are now
   implemented; preserve their cursor, privacy, storage-error and no-replay tests. Manual-run claims and owner-bound keyed replay are implemented;
   preserve their concurrent-admission, cancellation, ownership and malformed-receipt tests. Deleted one-shot recovery and declarative startup policies are
