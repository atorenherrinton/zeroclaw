...
| Cron jobs | Declarative config membership plus cron SQLite store | `data/cron/jobs.db` | `zeroclaw-runtime::cron` scheduler/store | Read paths do not create `jobs.db`; scheduler owns due/lock state. On in-process reload, a replacement scheduler waits for cron-owned workers using the same canonical DB path before clearing stale locks. A full process restart has no surviving process-local workers and clears dead claims at startup. | Declarative jobs are reconciled from config, while run metadata and locks live in the cron DB. |
