...
cli-eval-history-target-warning =   warning: the eval history directory is under target/ and may be cleared by other tooling; writing receipt anyway
cli-eval-history-write-warning =   warning: failed to write the eval history receipt (path and error details omitted); the eval result is unchanged
...
cli-eval-repeat-warning = {"  "}{$case} (repeat): {$warning}
cli-eval-repeat-warning-clamped-low = repeat < 1 clamped to 1
cli-eval-repeat-warning-clamped-high = repeat {$requested} clamped to 50
cli-eval-repeat-warning-replay-ignored = replay is deterministic; repeat ignored
cli-eval-repeat-error = {"  "}{$case} (repeat): repetition {$attempt}/{$total} errored, reporting {$completed} completed run(s): {$error}
cli-eval-repeat-scope = {$repeated} of {$total} cases have complete repeat statistics
cli-eval-repeat-ci-unavailable = repeated-case pass rate {$rate}% ({$scope}; 95% CI unavailable: insufficient independent units)
cli-eval-repeat-ci = repeated-case pass rate {$rate}% (95% CI {$lower}%–{$upper}%, {$scope})
cli-eval-repeat-case = repeat {$passes}/{$total}
cli-eval-repeat-completed = ({$completed} completed)
cli-eval-repeat-verdicts = pass@k {$pass_at}  pass^k {$pass_hat}
cli-eval-repeat-flips = flips: {$flips}
cli-eval-repeat-low-signal = 0/{$total}: low signal - possible invalid task or agent failure; rerun with higher repeat
cli-eval-repeat-suspect = suspect: 0/{$total} passes; inspect both task validity and agent behavior
