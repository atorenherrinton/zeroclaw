...
if ([...document.querySelectorAll('[aria-busy="true"]')].some(visible)) reasons.push('aria_busy');
if (args.selector) {
...
const controls = [...document.querySelectorAll(controlsSelector)].filter(visible).slice(0, 160);
...
const fingerprint = JSON.stringify([location.href, document.title, (document.body?.innerText || '').length,
